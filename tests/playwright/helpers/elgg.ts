...
export async function getLatestDiscussionByTitle(title: string): Promise<any | null> {
  const rows = await queryDb(
    `SELECT e.*
       FROM elgg_entities e
       JOIN elgg_metadata m ON m.entity_guid = e.guid
      WHERE e.type = 'object' AND e.subtype = 'discussion'
...
export async function getCommentsForEntity(containerGuid: number): Promise<any[]> {
  return queryDb(
    `SELECT * FROM elgg_entities
      WHERE type = 'object' AND subtype = 'comment' AND container_guid = ?`,
    [containerGuid]
  );
}
